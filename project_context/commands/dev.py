...
#     try:
#         session = AuthService.initialize_session(profile_override=use_profile)
#         api = session.api
#     except Exception as e:
#         UI.error(f"Error de autenticación o inicialización de sesión: {e}")
#         raise typer.Exit(1)

#     UI.info(f"Conectando al chat ID: [bold]{chat_id}[/]")

#     base_metadata = api.get_file_metadata(chat_id)
#     if not base_metadata:
#         UI.error("No se encontró el archivo. Verifica el ID y el perfil (--use).")
#         raise typer.Exit(1)

#     last_mod_time = base_metadata.get("modifiedTime")
#     raw_bytes = api.get_file_content(chat_id)
...
#             current_metadata = api.get_file_metadata(chat_id)
...
#     workspace.save_project_context_state(ProjectState(**old_state))
...
#         if api.delete_file(old_chat_id):
